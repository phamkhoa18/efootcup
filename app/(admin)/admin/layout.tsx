...
const adminSidebarLinks = [
    ...
    { label: "Quản lý giải đấu", href: "/admin/giai-dau", icon: Trophy },
    { label: "Bài viết & Tin tức", href: "/admin/bai-viet", icon: Newspaper },
    { label: "Danh mục", href: "/admin/danh-muc", icon: FolderTree },
    { label: "Quản lý Menu", href: "/admin/menu", icon: Globe },
    { label: "Cấu hình thanh toán", href: "/admin/thanh-toan", icon: CreditCard },
    { label: "Cài đặt hệ thống", href: "/admin/cai-dat", icon: Settings },
];

const adminBottomLinks = [
    { label: "Về trang Manager", href: "/manager", icon: Zap },
];
...
export default function AdminLayout({
    ...
    const currentLinks = adminSidebarLinks;
                ...
                <div className={`flex items-center gap-3 px-3 py-3 mt-2 rounded-xl bg-gray-50 ${collapsed ? "justify-center" : ""}`}>
                    ...
                    {!collapsed && (
                        <div className="flex-1 min-w-0">
                            <div className="text-sm font-semibold text-efb-dark truncate">{user?.name || "Admin"}</div>
                ...
                <header className="h-16 bg-white border-b border-gray-100 flex items-center justify-between px-6 flex-shrink-0 sticky top-0 z-30">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-50 border border-amber-200">
                            ...
                            <span className="text-[11px] font-bold text-amber-700 uppercase tracking-wider">Admin</span>
                        </div>
                                        ...
                                        notifications.map((notif: any) => (
                                            <DropdownMenuItem asChild key={notif._id}>
                                                <Link href={notif.link || "#"} className={`p-4 flex gap-3 items-start border-b border-gray-50 cursor-pointer rounded-none transition-colors ${notif.isRead ? "bg-white hover:bg-gray-50/80" : "bg-blue-50/30 hover:bg-blue-50/50"}`}>
                                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${notif.type === "registration" ? "bg-blue-100" : "bg-emerald-100"}`}>
