...
export default function SoDoThiDauPage() {
    ...
    const [gfRounds, setGfRounds] = useState<{ name: string; matches: any[] }[]>([]);
    ...
    const loadData = async () => {
        ...
        try {
            ...
            if (isDE) {
                ...
                setGfRounds(groupByBracket('grand_final'));
            } else {
                setWbRounds([]);
                setLbRounds([]);
                setGfRounds([]);
                ...
                <div id="bracket-capture-area" className="flex-1 overflow-auto rounded-[24px] border border-gray-100 relative custom-scrollbar shadow-inner bg-[#FDFDFD]">
                    ...
                    {isDoubleElimination ? (
                        <div className="flex flex-col gap-16 p-12 min-w-max relative z-10">
                            ...
                            {gfRounds.length > 0 && (
                                <div className="flex flex-col">
                                    <h3 className="text-base font-semibold uppercase tracking-wider text-purple-700 flex items-center gap-2 mb-6 pl-4">
                                        <Trophy className="w-5 h-5" /> Chung kết tổng
                                    </h3>
                                    <div className="pl-4">
                                        {renderBracketSection(gfRounds, 'gf')}
                                    </div>
                                </div>
                            )}
                        </div>
